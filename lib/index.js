...
function buildGlobs(manifest, options) {
  options = options || {};

  options.bowerDir = options.bowerJsonDirectory ? filePath(options.bowerJsonDirectory) : undefined;

  var bower = require('wiredep')(_.merge({
    exclude: _.map(obj.get(manifest, 'ignoreDependencies.bower'), function (val) {
      return new RegExp('\/' + val + '\/');
    })
  }, {
    cwd: options.bowerDir
  }));
  return {
    scripts: (bower.js || [])
    ...
    fonts: mainBowerFiles({ 
      filter: /\.(eot|svg|ttf|woff)$/i, 
      paths: options.bowerDir
    })
